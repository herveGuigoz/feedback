...
class TwoPane extends StatelessWidget {
  ...
  TextDirection _resolveTextDirection(BuildContext context) => textDirection ?? Directionality.of(context);

  @override
  Widget build(BuildContext context) {
    final resolvedTextDirection = _resolveTextDirection(context);
    ...
    const fractionBase = 1000000000000;

    if (mediaQuery == null) {
      resolvedDirection = direction;
      resolvedPanePriority = panePriority;
    } else {
      final position = padding.deflateRect(Offset.zero & mediaQuery.size);
      size = position.size;
      final separatingDisplayFeatures = _separatingDisplayFeatures(mediaQuery.displayFeatures, position);
      resolvedDirection = _resolveDirection(separatingDisplayFeatures, position);
      final displayFeature = _firstDisplayFeature(
    ...
    if (mediaQuery == null || resolvedPanePriority != TwoPanePriority.both) {
      ...
      final allowProportionOverride = allowedOverrides.contains(TwoPaneAllowedOverrides.paneProportion);
        ...
        case Axis.horizontal:
          {
            ...
            final leftFlex = (seam.left * fractionBase).toInt();
            final rightFlex = ((size.width - seam.right) * fractionBase).toInt();
            final leftScreen = Offset.zero & Size(seam.left, size.height);
            final rightScreen = seam.topRight & Size(size.width - seam.right, size.height);
        ...
        case Axis.vertical:
          {
            ...
            final topPane = (seam.top * fractionBase).toInt();
            final bottomPane = ((size.height - seam.bottom) * fractionBase).toInt();
            final topScreen = Offset.zero & Size(size.width, seam.top);
            final bottomScreen = seam.bottomLeft & Size(size.width, size.height - seam.bottom);
    ...
    return Flex(
      ...
      mainAxisAlignment:
          resolvedPanePriority != TwoPanePriority.both ? MainAxisAlignment.center : MainAxisAlignment.start,
      children: <Widget>[
        if (resolvedPanePriority != TwoPanePriority.end)
          KeyedSubtree.wrap(
            Expanded(flex: startPaneFlex, child: resolvedStartPane),
            1,
          ),
        if (resolvedPanePriority == TwoPanePriority.both) resolvedDelimiter,
        if (resolvedPanePriority != TwoPanePriority.start)
          KeyedSubtree.wrap(
            Expanded(flex: endPaneFlex, child: resolvedEndPane),
            2,
          ),
      ],
    );
  ...
  ///     [direction] is used.
  Axis _resolveDirection(Iterable<DisplayFeature> separatingFeatures, Rect position) {
    if (!allowedOverrides.contains(TwoPaneAllowedOverrides.direction)) {
      return direction;
    }
    var verticalSubScreensExist = false;
    var horizontalSubScreensExist = false;
    for (final displayFeature in separatingFeatures) {
  ...
  static DisplayFeature? _firstDisplayFeature(
    ...
    for (final displayFeature in displayFeatures) {
      switch (direction) {
        case Axis.horizontal:
          {
            if (!_splitsHorizontally(displayFeature.bounds, position)) {
  ...
  static bool _splitsHorizontally(Rect bounds, Rect position) {
    final tallEnough = bounds.top <= position.top && bounds.bottom >= position.bottom;
    final splitsHorizontally = bounds.left > position.left && bounds.right < position.right;
    return tallEnough && splitsHorizontally;
  }

  static bool _splitsVertically(Rect bounds, Rect position) {
    final wideEnough = bounds.left <= position.left && bounds.right >= position.right;
    final splitsVertically = bounds.top > position.top && bounds.bottom < position.bottom;
  ...
  static MediaQueryData _removeMediaQueryPadding(MediaQueryData data, Rect subScreen) {
    ...
    final rightInset = data.size.width - subScreen.right;
    final bottomInset = data.size.height - subScreen.bottom;
    return data.copyWith(
      padding: EdgeInsets.only(
        left: math.max(0, data.padding.left - subScreen.left),
        top: math.max(0, data.padding.top - subScreen.top),
        right: math.max(0, data.padding.right - rightInset),
        bottom: math.max(0, data.padding.bottom - bottomInset),
      ),
      viewPadding: EdgeInsets.only(
        left: math.max(0, data.viewPadding.left - subScreen.left),
        top: math.max(0, data.viewPadding.top - subScreen.top),
        right: math.max(0, data.viewPadding.right - rightInset),
        bottom: math.max(0, data.viewPadding.bottom - bottomInset),
      ),
      viewInsets: EdgeInsets.only(
        left: math.max(0, data.viewInsets.left - subScreen.left),
        top: math.max(0, data.viewInsets.top - subScreen.top),
        right: math.max(0, data.viewInsets.right - rightInset),
        bottom: math.max(0, data.viewInsets.bottom - bottomInset),
      ),
    );
  }
}
